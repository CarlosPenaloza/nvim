...
local function git_root()
	return firstline("git rev-parse --show-toplevel")
end
...
-- ===== Autor de la línea (git blame, sin popups) =====
local _blame_cache = { bufnr = nil, lnum = nil, tick = nil, ts = 0, text = "" }

local function is_normal_filebuf(buf)
	return vim.api.nvim_buf_is_loaded(buf)
		and vim.api.nvim_buf_get_name(buf) ~= ""
		and (vim.bo[buf].buftype or "") == ""
end

local function git_relpath_for(buf)
	...
	local root = git_root()
	if root and #root > 0 then
		local rel = vim.fn.fnamemodify(name, ":." .. root)
		if rel ~= name then
			return rel
		end
	end
...
function blame_author_current_line()
	local buf = vim.api.nvim_get_current_buf()
	if not is_normal_filebuf(buf) then
		return ""
	end
	...
	-- caché 1s
	if
		_blame_cache.bufnr == buf
		and _blame_cache.lnum == lnum
		and _blame_cache.tick == tick
		and (now - (_blame_cache.ts or 0) < 1000)
	then
		return _blame_cache.text or ""
	end

	local root = git_root()
	local author = ""
	if root and root ~= "" then
		local rel = git_relpath_for(buf)
		if rel ~= "" then
	...
	local text = (author ~= "" and author ~= "Not Committed Yet") and (" " .. author) or ""
	_blame_cache = { bufnr = buf, lnum = lnum, tick = tick, ts = now, text = text }
	return text
end
...
local function pick_primary_bg()
	local sel = get_hl("PmenuSel")
	if sel.bg then
		return hex(sel.bg)
	end
	local tab = get_hl("TabLineSel")
	if tab.bg then
		return hex(tab.bg)
	end
...
local function color_directory()
	local d = get_hl("Directory")
	if d.fg then
		return { fg = hex(d.fg) }
	end
	local id = get_hl("Identifier")
	if id.fg then
		return { fg = hex(id.fg) }
	end
	return {}
